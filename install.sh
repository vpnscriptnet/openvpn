#!/bin/bash
#
# Using without proper license is strictly prohibited.
# Contact: https://help.vpnscript.net/
# Help & documentation: https://docs.vpnscript.net/
# Copyright (c) 2021.  www.vpnscript.net
# V
#
...
prefetch: yes' >>/etc/unbound/unbound.conf
			...
			yum install -y unbound jq iptables
			...
			pacman -Syu --noconfirm unbound jq
...
function installQuestions() {
	...
	2)
		until [[ $PORT =~ ^[0-9]+$ ]] && [ "$PORT" -ge 1 ] && [ "$PORT" -le 65535 ]; do
			read -rp "Custom port [1-65535]: " -e -i 2504 PORT
		done
		;;
	3)
...
function installOpenVPN() {
	...
	if [[ ! -e /etc/openvpn/server.conf ]]; then
		if [[ $OS =~ (debian|ubuntu) ]]; then
			apt-get update
			apt-get -y install ca-certificates gnupg jq iptables
...
key $SERVER_NAME.key
auth-user-pass-verify /etc/vpnscript/authenticate.sh via-env
...
function manageMenu() {
	echo "Welcome to VPN Script OpenVPN Installer!"
